...
BEGIN_MESSAGE_MAP(CTrayDialog, CDialog)
	...
	ON_REGISTERED_MESSAGE(WM_TASKBARCREATED, TrayRedraw)
	ON_WM_CREATE()
	ON_WM_DESTROY()
	ON_WM_SYSCOMMAND()
	//}}AFX_MSG_MAP
...
void CTrayDialog::TraySetIcon(HICON hIcon)
{
	...
	m_nidIconData.uFlags = NIF_ICON | NIF_MESSAGE;
}
...
void CTrayDialog::TrayShowBalloonToolTip(int Type, const wchar_t* Title, const wchar_t* Info, int TimeMS)
{
	m_nidIconData.uFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP | NIF_INFO;
	m_nidIconData.dwInfoFlags = Type;

	m_nidIconData.uTimeout = TimeMS;

	wcsncpy_s(m_nidIconData.szInfoTitle, Title, 60);
	wcsncpy_s(m_nidIconData.szInfo, Info, 250);
...
LRESULT CTrayDialog::TrayRedraw(WPARAM wParam, LPARAM lParam)
{
	m_bTrayIconVisible = FALSE;
...
{
	SetForegroundWindow();
	m_mnuTrayMenu.TrackPopupMenu(TPM_BOTTOMALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON, pt.x, pt.y, this);
	//m_mnuTrayMenu.SetDefaultItem(m_nDefaultMenuItem, TRUE);
}
...
void CTrayDialog::OnTrayLButtonDblClk(CPoint pt)
{
	if (this->IsWindowVisible())
	{
		...
		else
		{
			...
			/*nit->Menu->SetDefaultItem(0, TRUE);
			TraySetMenu(Init->Menu->m_hMenu);*/
		}
	}
	...
	else
	{
		...
		/*Init->Menu->SetDefaultItem(1, TRUE);
		TraySetMenu(Init->Menu->m_hMenu);*/
	}
}
